<template>
  <div class="panel">
    <el-form status-icon ref="dormForm" :rules="rules" :model="dormForm" id="dormForm" label-width="80px">
      <el-form-item prop="name" label="楼名">
      ...
      <el-form-item prop="roomNo" label="房间号">
        <el-tag class="tag" type="warning" @close="handleClose2(item)" v-for="(item, index) in roomList" :key="index" closable>{{item}}</el-tag>
      ...
      <el-form-item prop="size" label="容量">
        <el-input type="size" v-model="dormForm.size"></el-input>
      </el-form-item>
      <el-form-item>
